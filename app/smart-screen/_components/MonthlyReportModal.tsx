...
export default function MonthlyReportModal() {
  ...
  const [vTotalA, setVTotalA] = useState(0);
  const [rTotalA, setRTotalA] = useState(0);
  const [oTotalA, setOTotalA] = useState(0);
  const [aovA, setAovA] = useState(0);
  const [vTotalB, setVTotalB] = useState(0);
  const [rTotalB, setRTotalB] = useState(0);
  const [oTotalB, setOTotalB] = useState(0);
  const [aovB, setAovB] = useState(0);
  ...
  const dataA: MonthlyReport | null = useMemo(() => {
    const ks = compareKeys;
    if (!ks) return null;
    const d = mockMonthlyReports[ks[0]];
    return d ? d : null;
  }, [compareKeys]);
  const dataB: MonthlyReport | null = useMemo(() => {
    const ks = compareKeys;
    if (!ks) return null;
    const d = mockMonthlyReports[ks[1]];
    return d ? d : null;
  }, [compareKeys]);
  ...
  useEffect(() => {
    const off = onPanelAction((a) => {
      ...
      } else if (a && (a as { kind?: string }).kind === "report:compare") {
        const ms = (a as { months?: [string, string] }).months;
        const ok = Array.isArray(ms) && ms.length === 2 && typeof ms[0] === "string" && typeof ms[1] === "string" && mockMonthlyReports[ms[0]] && mockMonthlyReports[ms[1]];
        if (ok) {
          setCompareKeys([ms[0], ms[1]]);
          setMode("compare");
          setMonthKey("");
          setOpen(true);
          setReady(false);
  ...
  useEffect(() => {
    ...
    if (mode === "single") {
      ...
      const tick = (ts: number) => {
        ...
        setVTotal(Math.floor(d.visitorsTotal * ease));
        setRTotal(Math.floor(d.revenueTotal * ease));
        setOTotal(Math.floor(d.ordersTotal * ease));
        setAov(Math.floor(d.avgOrderValue * ease));
    ...
    if (mode === "compare") {
      ...
      const tick = (ts: number) => {
        const p = Math.min(1, (ts - t0) / dur);
        const ease = 0.5 - Math.cos(Math.PI * p) / 2;
        setVTotalA(Math.floor(a.visitorsTotal * ease));
        setRTotalA(Math.floor(a.revenueTotal * ease));
        setOTotalA(Math.floor(a.ordersTotal * ease));
        setAovA(Math.floor(a.avgOrderValue * ease));
        setVTotalB(Math.floor(b.visitorsTotal * ease));
        setRTotalB(Math.floor(b.revenueTotal * ease));
        setOTotalB(Math.floor(b.ordersTotal * ease));
        setAovB(Math.floor(b.avgOrderValue * ease));
        if (p < 1) requestAnimationFrame(tick);
      };
      requestAnimationFrame(() => {
        setVTotalA(0); setRTotalA(0); setOTotalA(0); setAovA(0);
        setVTotalB(0); setRTotalB(0); setOTotalB(0); setAovB(0);
        setReady(true);
        requestAnimationFrame(tick);
      });
    }
  }, [open, mode, monthKey, compareKeys, data, dataA, dataB]);

  if (!open) return null;
  if (mode === "single" && !data) return null;
  if (mode === "compare" && (!dataA || !dataB)) return null;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-[92%] max-w-[1080px] rounded-2xl bg-[#0b1220] ring-1 ring-white/10 shadow-2xl overflow-hidden animate-modal-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <div>
            <div className="text-xs text-zinc-400">{mode === "compare" ? "月度对比" : "月度详细报表"}</div>
            <div className="text-lg font-semibold tracking-wider">
              {mode === "compare" && dataA && dataB ? (
                <span>{monthLabelA} · {dataA.monthKey}  vs  {monthLabelB} · {dataB.monthKey}</span>
              ) : (
                <span>{monthLabel} · {data ? data.monthKey : ""}</span>
              )}
            </div>
          </div>
          <button
            ...
            <>
              <div className="col-span-1 grid grid-cols-2 gap-4">
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  ...
                  <div className="mt-2 text-2xl font-semibold">{vTotalA.toLocaleString()} <span className="text-xs text-zinc-500">人</span></div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">总金额</div>
                  <div className="mt-2 text-2xl font-semibold">¥ {rTotalA.toLocaleString()}</div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">订单数</div>
                  <div className="mt-2 text-2xl font-semibold">{oTotalA.toLocaleString()}</div>
                  <div className="text-[10px] text-zinc-500">转化率 {Math.round(dataA.conversionRate)}%</div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">平均客单价</div>
                  <div className="mt-2 text-2xl font-semibold">¥ {aovA.toLocaleString()}</div>
                </div>
              </div>
              <div className="col-span-1 grid grid-cols-2 gap-4">
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">总人数</div>
                  <div className="mt-2 text-2xl font-semibold">{vTotalB.toLocaleString()} <span className="text-xs text-zinc-500">人</span></div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">总金额</div>
                  <div className="mt-2 text-2xl font-semibold">¥ {rTotalB.toLocaleString()}</div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">订单数</div>
                  <div className="mt-2 text-2xl font-semibold">{oTotalB.toLocaleString()}</div>
                  <div className="text-[10px] text-zinc-500">转化率 {Math.round(dataB.conversionRate)}%</div>
                </div>
                <div className="rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                  <div className="text-xs text-zinc-400">平均客单价</div>
                  <div className="mt-2 text-2xl font-semibold">¥ {aovB.toLocaleString()}</div>
                </div>
              </div>
              <div className="col-span-1 rounded-xl bg-black/30 p-5 ring-1 ring-white/10">
                <div className="text-xs text-zinc-400">对比分析</div>
                <div className="mt-4 space-y-3 text-sm text-zinc-200">
                  <div>营收 {Math.round(((dataB.revenueTotal - dataA.revenueTotal) / Math.max(1, dataA.revenueTotal)) * 100)}% {dataB.revenueTotal - dataA.revenueTotal >= 0 ? "↑" : "↓"}</div>
                  <div>人数 {Math.round(((dataB.visitorsTotal - dataA.visitorsTotal) / Math.max(1, dataA.visitorsTotal)) * 100)}% {dataB.visitorsTotal - dataA.visitorsTotal >= 0 ? "↑" : "↓"}</div>
                  <div>订单 {Math.round(((dataB.ordersTotal - dataA.ordersTotal) / Math.max(1, dataA.ordersTotal)) * 100)}% {dataB.ordersTotal - dataA.ordersTotal >= 0 ? "↑" : "↓"}</div>
                  <div>客单价 {Math.round(((dataB.avgOrderValue - dataA.avgOrderValue) / Math.max(1, dataA.avgOrderValue)) * 100)}% {dataB.avgOrderValue - dataA.avgOrderValue >= 0 ? "↑" : "↓"}</div>
                  <div>小程序占比 {Math.round((dataB.channels.miniapp - dataA.channels.miniapp))}% {dataB.channels.miniapp - dataA.channels.miniapp >= 0 ? "↑" : "↓"}</div>
                  <div className="mt-3 text-[12px] text-zinc-400">建议：加大小程序投放、优化漏斗节点、峰值时段增配互动。</div>
                </div>
              </div>
            </>
          ) : (
